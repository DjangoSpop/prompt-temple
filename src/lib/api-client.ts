...
class ApiClient {
  ...
  async getTemplateAnalytics(templateId: string): Promise<any> {
    try {
      const response = await this.axiosInstance.get(`/api/templates/${templateId}/analytics/`);
      return response.data;
    } catch (error) {
  ...
  async duplicateTemplate(templateId: string, customizations?: any): Promise<TemplateDetail> {
    try {
  ...
  async trackEvent(eventData: AnalyticsEvent): Promise<void> {
    try {
      await this.axiosInstance.post('/api/analytics/events/', {
        ...eventData,
